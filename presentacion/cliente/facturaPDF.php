...
$tel = new TelCliente($_SESSION["id"], "", "");
$tel -> consultar();
...
$pdf->Cell(5,$textypos, $tel -> getTelefono());
...
$pdf->Cell(5,$textypos,"FACTURA" . $factura -> getIdFactura());
$pdf->SetFont('Arial','',10);
...
$productosFactura = new Orden("", "", $pIdCliente = "", "", "", "", $numOrden, "" );
$productosFactura -> productosFacturaOrden();
...
for($i=0;$i<count($header);$i++)
    ...
    $total = 0;
    $j = 0;
    foreach($productosFactura as $producActual)
    {
        $pdf->Cell($w[0],6,$j++,1);
        $pdf->Cell($w[1],6,$producActual -> getIdProducto() -> getIdMarca() -> getNombre(),1);
